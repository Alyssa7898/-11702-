...
public class EmployeeController {
	...
	@RequestMapping(value="/toempInfo",method=RequestMethod.GET)
	public String toempInfo(Employee employee,HttpServletRequest request,HttpSession session) {
		String username=(String) session.getAttribute("username");
		System.out.println("邮箱："+username);
		 employee=employeeService.selectByEmailWithDep(username);
//		String emp_name=employee.getEmpName();
//		String gender=employee.getGender();
//		String email=employee.getEmail();
//		Integer deptno=employee.getdId();
//		String dept=deptno.toString();
...
//		request.setAttribute(dept,"dept");
	...
	@RequestMapping("/empsign")
	public String empsign(HttpServletRequest request,HttpSession session) {
		String email=(String) session.getAttribute("email");
		SimpleDateFormat formatter = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss"); 
		System.out.println("当前系统时间："+formatter.format(new java.sql.Date(new java.util.Date().getTime())));
		TimeZone time = TimeZone.getTimeZone("ETC/GMT-8"); 
		TimeZone.setDefault(time);
		System.out.println("修改后时间 " + formatter.format(new java.sql.Date(new java.util.Date().getTime())));
		String edate=formatter.format(new java.sql.Date(new java.util.Date().getTime())).toString();
		employeeService.updateByEmail(email,edate);
		request.setAttribute("msg", "签到成功！祝您工作愉快！");
		return "msg";
